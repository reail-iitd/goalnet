...
parser.add_option("-s", "--save", action="store_true", dest="save_json", default=False,
                  help="run with stopping criterion = GT plan length")
parser.add_option("--no_relation", action="store_true", dest="no_relational_info", default=False,
                  help="no relational information in input world graph")
parser.add_option("--rintanen", action="store_true", dest="use_rintanen", default=False,
                  help="train using rintanen after 50 epochs")
parser.add_option("--no_instance_grounding", action="store_true", dest="no_instance_grounding", default=False,
                  help="if one wants to train without instance grounding information.")
parser.add_option("--no_grammar_mask", action="store_true", dest="no_gammar_mask", default=False,
                  help="if you want to run code without the grammar masking on prediction")
parser.add_option("--conceptnet", action="store_true", dest="use_conceptnet", default=False,
                  help="if you want to train with conceptnet instead of SBERT")
parser.add_option("--no_temporal_context", action="store_true", dest="no_temporal_context", default=False,
                  help="if you want to train without temporal context")
parser.add_option("--temporal_context_delta", action="store_true", dest="temporal_context_delta", default=False,
                  help="training with delta as temporal context")
parser.add_option("--temporal_context_state", action="store_true", dest="temporal_context_state", default=False,
                  help="training with delta as temporal context")
...
def vect2string(state_dict, action, pred1_object, pred2_object, pred2_state, env_domain, arg_map = None):
    ...
    if opts.no_gammar_mask:
        obj1_index = torch.argmax(pred1_object).item()
    else:
