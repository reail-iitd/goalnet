...
(:action press_microwave_ovenButton :parameters ()
:precondition (Near Robot microwave_oven)
:effect (and (when (not (state microwave_oven MicrowaveIsOn))
			       (and (state microwave_oven MicrowaveIsOn) (forall (?otherobj)
													   (when (In microwave_oven ?otherobj)
															 (state ?otherobj temperatureHigh)
													   )
											   ))
              )
			  (when (state microwave_oven MicrowaveIsOn)
			       (not (state microwave_oven MicrowaveIsOn))
              )
	    )
)
...
; Press refrigerator water dispenser. If close then turn and fill grasped objects else close

(:action press_refrigeratorButton :parameters ()
:precondition (Near Robot refrigerator)
:effect (and (when (not (state refrigerator WaterDispenserIsOpen))
			       (and (state refrigerator WaterDispenserIsOpen) (forall (?otherobj)
														            (when (Grasping Robot ?otherobj)
															              (state ?otherobj Water)
													                )
											                ))
             )
			 (when (state refrigerator WaterDispenserIsOpen)
			       (not (state refrigerator WaterDispenserIsOpen))
             )
	    )
)
...
(:action turn_wash_basinKnob :parameters ()
:precondition (Near Robot wash_basin)
:effect (and (when (not (state wash_basinKnob TapIsOn))    
                        (and (state wash_basinKnob TapIsOn)  (forall (?otherobj)
												            (when (On ?otherobj wash_basin)
														          (state ?otherobj Water)
												            )
				                                    )
					    ) 
		     )
			 (when (state wash_basinKnob TapIsOn)    
						 (not (state wash_basinKnob TapIsOn))
		     )
		)
)
...
(:action turn_kitchen_stoveKnob_1 :parameters ()
:precondition (Near Robot kitchen_stove)
:effect (and (when (not (state kitchen_stove StoveFire1))    
                        (and (state kitchen_stove StoveFire1)  (forall (?otherobj)
												            (when (On ?otherobj StoveFire1)
														          (state ?otherobj temperatureHigh)
												            )
				                                       )
			 ...
			 (when (state kitchen_stove StoveFire1)    
						 (not (state kitchen_stove StoveFire1))
		     )
		)
)
...
(:action turn_kitchen_stoveKnob_2 :parameters ()
:precondition (Near Robot kitchen_stove)
:effect (and (when (not (state kitchen_stove StoveFire2))    
                        (and (state kitchen_stove StoveFire2)  (forall (?otherobj)
												            (when (On ?otherobj StoveFire2)
														          (state ?otherobj temperatureHigh)
												            )
				                                       )
			 ...
			 (when (state kitchen_stove StoveFire2)    
						 (not (state kitchen_stove StoveFire2))
		     )
		)
)
...
(:action turn_kitchen_stoveKnob_3 :parameters ()
:precondition (Near Robot kitchen_stove)
:effect (and (when (not (state kitchen_stove StoveFire3))    
                        (and (state kitchen_stove StoveFire3)  (forall (?otherobj)
												            (when (On ?otherobj StoveFire3)
														          (state ?otherobj temperatureHigh)
												            )
				                                       )
			 ...
			 (when (state kitchen_stove StoveFire3)    
						 (not (state kitchen_stove StoveFire3))
		     )
		)
)
...
(:action turn_kitchen_stoveKnob_4 :parameters ()
:precondition (Near Robot kitchen_stove)
:effect (and (when (not (state kitchen_stove StoveFire4))    
                        (and (state kitchen_stove StoveFire4)  (forall (?otherobj)
												            (when (On ?otherobj StoveFire4)
														          (state ?otherobj temperatureHigh)
												            )
				                                       )
			 ...
			 (when (state kitchen_stove StoveFire4)    
						 (not (state kitchen_stove StoveFire4))
		     )
		)
)
...
(:action open_refrigeratorLeftDoor :parameters ()
:precondition (and (Near Robot refrigerator) (not (state refrigerator LeftDoorIsOpen)))
:effect (and (state refrigerator LeftDoorIsOpen)
             (not (state refrigerator WaterDispenserIsOpen))
	    )
)

(:action open_refrigeratorRightDoor :parameters ()
:precondition (and (Near Robot refrigerator) (not (state refrigerator RightDoorIsOpen)))
:effect (state refrigerator RightDoorIsOpen)
)
...
(:action close_refrigeratorLeftDoor :parameters ()
:precondition (and (Near Robot refrigerator) (state refrigerator LeftDoorIsOpen))
:effect (not (state refrigerator LeftDoorIsOpen) ))

(:action close_refrigeratorRightDoor :parameters ()
:precondition (and (Near Robot refrigerator) (state refrigerator RightDoorIsOpen))
:effect (not (state refrigerator RightDoorIsOpen)))
...
(:action keep_On_StoveFire1 :parameters (?x)
:precondition (and (Grasping Robot ?x) (Near Robot kitchen_stove))
:effect (and (not (Grasping Robot ?x)) (On ?x StoveFire1) (when (state kitchen_stove StoveFire1)
																  (state ?x temperatureHigh)
                                                            )
        )
)
...
(:action keep_On_StoveFire2 :parameters (?x)
:precondition (and (Grasping Robot ?x) (Near Robot kitchen_stove))
:effect (and (not (Grasping Robot ?x)) (On ?x StoveFire2) (when (state kitchen_stove StoveFire2)
																  (state ?x temperatureHigh)
                                                            )
        )
)
...
(:action keep_On_StoveFire3 :parameters (?x)
:precondition (and (Grasping Robot ?x) (Near Robot kitchen_stove))
:effect (and (not (Grasping Robot ?x)) (On ?x StoveFire3) (when (state kitchen_stove StoveFire3)
																  (state ?x temperatureHigh)
                                                            )
        )
)
...
(:action keep_On_StoveFire4 :parameters (?x)
:precondition (and (Grasping Robot ?x) (Near Robot kitchen_stove))
:effect (and (not (Grasping Robot ?x)) (On ?x StoveFire4) (when (state kitchen_stove StoveFire4)
																  (state ?x temperatureHigh)
                                                            )
        )
)
...
; Keeping something on the wash_basin

(:action keep_On_wash_basin :parameters (?x)
:precondition (and (Grasping Robot ?x) (Near Robot wash_basin))
:effect (and (not (Grasping Robot ?x)) (On ?x wash_basin) (when (state wash_basinKnob TapIsOn)
																  (and (state ?x Water) (not (state ?x temperatureHigh)))
                                                            )
		)
)
...
; keep for refrigerator

(:action keep_In_refrigeratorLeft :parameters (?x)
:precondition (and (Grasping Robot ?x) (Near Robot refrigerator) (state refrigerator LeftDoorIsOpen))
:effect (and (not (Grasping Robot ?x)) (In ?x refrigeratorLeft) ) )

(:action keep_In_refrigeratorRight :parameters (?x)
:precondition (and (Grasping Robot ?x) (Near Robot refrigerator) (state refrigerator RightDoorIsOpen))
:effect (and (not (Grasping Robot ?x)) (In ?x refrigeratorRight) ) )
...
; keeping for bin_liner

(:action keep_In_bin_liner_1 :parameters (?x)
:precondition (and (Grasping Robot ?x)  (Grasping Robot bin_liner_1))
:effect (and (not (Grasping Robot ?x)) (In ?x bin_liner_1) ))
...
:effect  (and (not (state ?x Water)) (not (state ?x Coffee)) (not (state ?x Coke))
              ...
              (not (state ?x sports_drink)) (not (state ?x popular_soda)) (not (state ?x instant_noodle)) 
		 )
)
...
:effect  (and (when (state ?x Water)
              ...
              (when (state ?x instant_noodle)
					(state instant_noodle ?y)
              )
              ...
              (when (state ?x sports_drink)
					(state sports_drink ?y)
              )

              (when (state ?x popular_soda)
					(state popular_soda ?y)
              )
		  )
)
...
(:action add_instant_noodle_1 :parameters(?x)
:precondition (and (Grasping Robot instant_noodle_1) (Grasping Robot ?x))
:effect (and (state ?x instant_noodle) (not (Grasping Robot instant_noodle_1)) (In instant_noodle_1 ?x))
)
...
(:action add_salted_egg_1 :parameters(?x)
:precondition (and (Grasping Robot salted_egg_1) (Grasping Robot ?x))
:effect (and (state ?x Egg) (not (Grasping Robot salted_egg_1)) (In salted_egg_1 ?x))
)
...
(:action add_IceCreamScoop :parameters(?x)
:precondition (and (Grasping Robot big_spoon_1) (state big_spoon_1 ScoopsLeft))
:effect (and (state ?x IceCream) (not (state big_spoon_1 ScoopsLeft)) )
)

(:action add_frozen_dessertScoop :parameters(?x)
:precondition (and (Grasping Robot Spoon_1) (state Spoon_1 ScoopsLeft))
:effect (and (state ?x IceCream) (not (state Spoon_1 ScoopsLeft)) )
)
...
(:action place_tablefork_1 :parameters(?y)
:precondition (Grasping Robot ?y)
:effect (and (not (Grasping Robot ?y)) (state ?y tablefork) (In tablefork_1 ?y))
)
...
(:action place_big_spoon_1 :parameters(?y)
:precondition (Grasping Robot ?y)
:effect (and (not (Grasping Robot ?y)) (state ?y big_spoon) (In big_spoon_1 ?y))
)
